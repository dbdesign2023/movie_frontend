    ...
    <div className='form-container'>
      <div className='row'>
        ...
        <div className="col-sm-4 justify-content-start">
          {clickcheck && !name && <div style={{color:'red'}}>이름을 입력하세요.</div>}
        </div>
      </div>
      <div className='row'>
        <div className="col-sm-4">
        ...
        <div className="col-sm-4">
          <input type="text" className="form-control" value={login_id} onChange={(event) => setId(event.target.value)}/>
        </div>
        <div className="col-sm-4 justify-content-start">
          {clickcheck && !login_id && <div style={{color:'red'}}>아이디를 입력하세요.</div>}
        </div>
      </div>
      <div className='row'>
        <div className="col-sm-4">
        ...
        <div className="col-sm-4 justify-content-start">
          {clickcheck && !password && <div style={{color:'red'}}>비밀번호를 입력하세요.</div>}
        </div>
      </div>
      <div className='row'>
        <div className="col-sm-4">
        ...
        <div className="col-sm-4 justify-content-start">
          {clickcheck && !nickname && <div style={{color:'red'}}>닉네임을 입력하세요.</div>}
        </div>
      </div>
      <div className='row'>
        <div className="col-sm-4">
        ...
        <div className="col-sm-4 justify-content-start">
          {clickcheck && !birthdate && <div style={{color:'red'}}>생년월일을 선택하세요.</div>}
        </div>
      </div>
      <div className='row'>
        <div className="col-sm-4">
        ...
        <div className="col-sm-4">
          <label>
            <input type="radio" value="1" checked={gender === "1"} onChange={handleClickRadioButton}/>
            남성
          </label>
          <label className="ms-auto">
            <input type="radio" value="2" checked={gender === "2"} onChange={handleClickRadioButton}/>
            여성
          </label>
        </div>
        <div className="col-sm-4 justify-content-start">
          {clickcheck && !gender && <div style={{color:'red'}}>성별을 선택하세요.</div>}
        </div>
      </div>
      <div className='row'>
        <div className="col-sm-4">
        ...
        <div className="col-sm-4">
          <input type="text" className="form-control" placeholder="01012345678" value={phone_number} onChange={(event) => setPhonenumber(event.target.value)}/>
        </div>
        <div className="col-sm-4 justify-content-start">
          {clickcheck && !check_phone_number.test(phone_number) && <div style={{color:'red'}}>유효한 전화번호를 입력하세요.</div>}
        </div>
      </div>
      <div className='row'>
        <div className="col-sm-4">
        ...
        <div className="col-sm-4">
          <input type="email" className="form-control" placeholder="name@example.com" value={email} onChange={(event) => setEmail(event.target.value)}/>
        </div>
        <div className="col-sm-4 justify-content-start">
          {clickcheck && !check_email.test(email) && <div style={{color:'red'}}>유효한 이메일을 입력하세요.</div>}
        </div>
      </div>
      <button className="btn btn-primary m-1" onClick={registerHandler}>회원가입</button>
      <Modal
        visible={registerfin}
        effect='fadeInDown'
        onClickAway={closemodal}>
            <div className='title-text-center-container'>회원가입이 완료되었습니다!</div>
