...
export default function TicketingSeatForm() {
  const seat = `${process.env.PUBLIC_URL}/img/seat.png`;
  const seat_gray = `${process.env.PUBLIC_URL}/img/seat_gray.png`;
  const seat_choose = `${process.env.PUBLIC_URL}/img/seat_choosepng.png`;
  const ip = baseUrl;
  ...
  const reservation = (seat) => {
    if (seat.row) {
      ...
      if (!seatlist.includes(string)) {
        pdata[seat.row.charCodeAt() - 'A'.charCodeAt()].ticketed[
          seat.col - 1
        ] = 2;
  ...
  const mem_post_ticket = async () => {
    try {
      ...
      const response = await axios.post(url, datatmp, header);
              ...
              <div key={idx1} className='row justify-content-start'>
                {item.ticketed.map((tmp, idx2) => (
                  ...
                  >
                    <img
                      src={
                        tmp === 0 ? seat : tmp === 1 ? seat_gray : seat_choose
                      }
                      onClick={
                        tmp === 0
                          ? chooseSeat.bind({
                              col: item.col_num[idx2],
                              row: item.row_num,
                            })
                          : tmp === 1
                          ? choosenSeat
      ...
      <div>
        ...
        {modify && (
          <button
            type='button'
            className='btn btn-primary m-1'
            onClick={membermodifyhandler}
          >
            수정
          </button>
        )}
      </div>
      <Modal visible={modal} effect='fadeInDown' onClickAway={closeModal}>
        ...
        <div style={{ padding: 10 }}>
          ...
          {modify && (
            <button
              type='button'
              className='btn btn-primary m-1'
              onClick={nonmembermodifyhandler}
            >
              티켓 수정하기
            </button>
          )}
